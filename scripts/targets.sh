...
#  installers look for: `rune-<suffix>` and `rune-tools-<suffix>`.
#
#  Usage:
#    source scripts/targets.sh
#    for pair in "${RUNE_TARGETS[@]}"; do target="${pair%%:*}"; suffix="${pair##*:}"; done
# ──────────────────────────────────────────────────────────
...
# The rune-tools assets a complete release carries, in the same suffix
# vocabulary. `rune-tools` is Rust and cannot be cross-compiled by bun, so it
