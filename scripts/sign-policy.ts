#!/usr/bin/env bun
/**
 * Org-admin tool: sign a Berne org policy for distribution to managed machines.
 *
 ...
 * and writes:
 *
 *   <out-dir>/policy.json   — { policy, signature }  → install at /etc/berne/policy.json
 *   <out-dir>/org.pub       — public key (PEM)       → install at /etc/berne/org.pub
 ...
 * /Library/Application Support/Berne/ instead. Berne refuses to start if the
...
function main(): void {
  ...
  console.log("install both at /etc/berne/ (root-owned) on managed machines.");
}
