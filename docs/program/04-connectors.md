...
`gear mcp add notion` authenticates in the browser and the next session can call Notion. A 40-tool server does not cost 40 schemas per request. A third party can publish a plugin and a user can install it from a URL.
...
- Config is `.gear/mcp.json` in the workspace (`discovery.ts:120`); no user scope; no `[mcp]` section in `GearConfig` (`packages/shared/src/config.ts:8-470`). No `gear mcp` command; `/mcp` is read-only and tells the user to hand-edit the file (`gear-cli.ts:1960-1985`).
- Failures are silent under the TUI: `McpClient` emits a typed `McpEvent` stream (`types.ts:89-102`) that the engine never subscribes to (`engine.ts:1687-1693` passes no `onEvent`), and `logger.ts:101` suppresses stderr while `GEAR_TUI_ACTIVE=1`. The model is told nothing when a server is down.
...
- `custom-loader.ts` (210 lines) is dead code: exported, tested, never instantiated. `plugins.ts` supports `.gear/plugins/<name>/plugin.json` with hooks, mcp, commands, auto-discovered skills; no install command; `PluginDiscovery.errors` (`plugins.ts:52-54`) are never displayed; no `gearVersion`, permissions or integrity fields.
...
gear mcp add notion && gear mcp login notion     # browser OAuth completes, token in the credential store
gear -P --stream-json "list my three most recent Notion pages" | grep '"tool_call_start".*mcp_notion'   # live, behind GEAR_LIVE_CONNECTOR_TEST=1
gear audit last | grep "schema tokens"           # per-request schema tokens reported; ≥40% lower than pre-P4.1 baseline with two connectors
gear plugin add https://github.com/<org>/<demo-plugin> && gear mcp list   # plugin contributes a skill and an MCP server
gear mcp doctor                                   # a stopped server shows as down in the TUI and the desktop
```
