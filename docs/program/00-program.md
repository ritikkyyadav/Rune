# Rune — The Product Program

**Rev 01 · 2026-09-02 · written at `b1901e9` +53 dirty · for execution by a frontier model (Opus 5) with one developer**

> **Revision 2026-09-03 (evening) — the product is the terminal.** After seeing the web app and the intent-layer shell, the founder decided Rune's interface is the terminal console and nothing else. `apps/web`, the bundle embedded in the binary, `rune web` / `rune open`, the VS Code extension and the Playwright suite are removed; `rune serve`, `rune attach ws://`, ACP and the SDK stay as headless protocol surfaces. The same day the product was renamed **Gear → Rune** (the permission ladder keeps its "gears") and the accent became the founder's violet `#A28CF3`. Phases 3, 5.2, 9 and 11.2–11.3 are withdrawn; 11.1 (task state, narrative, Decision Record) stays as engine work. See [status.md](status.md).
...
> **Revision 2026-09-02 (evening).** Phase 3's native desktop shell and the Savoir visual system were a misunderstanding: the product surface is one URL-based web app in the current blue-rune identity, identical on every OS. See [09-web-product.md](09-web-product.md). Phases 1–8 landed as draft PRs #4–#12 the same day; see [status.md](status.md).

> Rune is a better agent than OpenCode and a far worse product. Capability index 110, maturity 44, blended 84 against OpenCode at 100 (`docs/history` scorecard, 2026-09-02). This program turns the harness into a product without touching the parts that already win.
...
5. **Reinstall and smoke after each phase.** `scripts/install.sh` from the main checkout, then `rune doctor` and `rune tools-smoke`. Never set `RUNE_ALLOW_NON_FF_INSTALL`.
...
7. **No new UI dialect.** Every surface consumes `@rune/protocol`. No surface reaches into engine internals. The TUI's `ui-grammar.test.ts` stays green and the desktop gets an equivalent.
...
2. **Proof.** Steps close on evidence, not on the model's say-so (0 of 87 completed steps unproven across 30 days). Every answer traces to the tools, prompts, permissions and policy that produced it (`rune audit`, the desktop trace rail). A black box records every failure. Exports are signed.

What Rune declines: a hosted cloud, a proprietary model, a marketplace, a chat toy. It is local-first and auditable, and it says so.
...
**Desktop app or CLI?** Both, because the engine is already a daemon. Make the protocol official (`@rune/protocol`, `rune serve`), then every UI is a client: the terminal, the desktop, the same UI in a browser (`rune web`), editors through ACP, CI through `rune -P --stream-json`. The desktop is the flagship UX investment because the trace rail lives there and because the terminal medium has been through ten UI programs and still reads as "stuck in between." The maintenance-surface worry is answered structurally: one protocol package (no duplicated event unions), a generic passthrough bridge in Rust (no per-command drift), and a web-first UI (Tauri is a wrapper; Linux and remote users get `rune web`). What remains is code signing on two platforms and an updater: three to four days to set up, about a day a month after.
...
**Tooling and connectors.** `rune mcp add notion` with OAuth 2.1, a seed catalog from the 20 vendored connector templates plus the public MCP registry, deferred tool schemas so a 40-tool server does not cost 40 schemas per request, and plugin install from a URL. Phase 4.
...
```
                         ┌──────────────────────────────────┐
   packages/orchestrator │  ENGINE  (unchanged core)        │  crates/rune-sandbox
   packages/tool-registry│  agent loop · context · gears    │  crates/rune-tools
   packages/llm-gateway  │  verifier · ledger · black box   │  crates/rune-index
                         └───────────────┬──────────────────┘
                                         │ AgentTurnEvent (22) · ResearchEvent (9)
                                         │ 25 commands · 5 handler round-trips
                         ┌───────────────┴──────────────────┐
                         │  @rune/protocol  (Phase 2)       │
                         │  JSON-RPC 2.0 · versioned        │
                         │  stdio │ unix socket │ websocket │
                         └───────────────┬──────────────────┘
          ┌──────────┬──────────┬────────┴───────┬──────────┬──────────┐
       rune (TUI)  Rune Desktop  rune web      editors    rune -P     @rune/sdk
...
| --- | ---------------------------------------------------- | ---- | ----- | ------------------------------------------------------------------------------------------------- |
| 1   | [Ship](01-ship.md)                                   | A    | 6–9   | Fresh machine, one command, three OSes, `rune doctor` clean; `gh release view v0.3.0`             |
...
| 4   | [Connect anything](04-connectors.md)                 | C    | 12–16 | `rune mcp add notion` → browser OAuth → tool call in a session; schema tokens/request reported    |
| 5   | [Everywhere the work happens](05-surfaces.md)        | B    | 8–12  | Zed drives a session over ACP; a GitHub workflow in this repo runs Rune on a PR                   |
...
**Cut line at week 6.** If the program stops there, the world has: v0.3.x installable on three OSes, `rune serve`, a desktop that launches and consumes the protocol (3.1–3.3), and connectors that authenticate (4.1–4.3). That is already a product.
...
What Claude Code has, and what Rune does about each. "Have" means it exists and works; "build" names the phase.

| Surface                                                      | Rune                            | Action                                            |
| ------------------------------------------------------------ | ------------------------------- | ------------------------------------------------- |
| Terminal CLI                                                 | have                            | freeze as the console (3.7)                       |
| Desktop app (macOS, Windows)                                 | have, cannot launch             | build, plus Linux (3)                             |
| Web app                                                      | none                            | build `rune web` from the same bundle (3.2)       |
...
| GitHub Actions                                               | none                            | build `rune-action` (5.3)                         |
| Agent SDK                                                    | 80% (`HostClient`)              | build `@rune/sdk` (5.4)                           |
...
| Multi-agent code review                                      | pattern exists                  | defer; a `rune review` workflow is a 6B stretch   |
| Remote control                                               | serve `--host`                  | build (5.5); cloud decline                        |
| Multi-provider, local models                                 | have                            | Rune-only differentiator                          |
| Audit page, trace rail, black box, signed export, org policy | have                            | Rune-only differentiator; make them reachable (3) |
...
- **D2 Brand.** ~~The Savoir brand DNA…~~ **Answered 2026-09-02 and executed in Phase 9:** the Savoir DNA was an identity the founder had already ditched, and the current one is a solid electric-blue eight-tooth rune on a near-white ground (`#1B3FE4` / `#FAFAF8`, Geist + Geist Mono). The system is [`docs/design/web/DIRECTION.md`](../design/web/DIRECTION.md); the mark is generated from geometry in `apps/web/branding/`. Still open for the founder: the original vector (to replace the recreation byte-for-byte), the story copy for first-run and the download page, and whether the product is "Rune" or "Savoir Rune".
- **D3 Default surface.** ~~`rune` stays the terminal…~~ **Answered 2026-09-02 and executed in Phase 9:** there is no native desktop app. `rune` opens the web app on a local URL; `rune --console` (alias `rune tui`) is the terminal; the download page leads with the app. See [09-web-product.md](09-web-product.md).
- **D4 Windows.** **Recommend:** ship the CLI for Windows with an honest banner ("no OS sandbox on Windows; 1st and 2nd gear only, or run under WSL2"). A native Windows sandbox is deferred.
...
- `02-protocol.md` — `@rune/protocol`; `rune serve`; all five handlers as round-trips; replay; sub-agent event fidelity; `--stream-json`.
- `03-desktop.md` — revive and finish the desktop; `rune web`; the Savoir design system as the one token source; packaging and updater; the TUI freeze.
- `04-connectors.md` — deferred tool loading; MCP OAuth 2.1; `rune mcp`; resources/prompts/elicitation; failure surfacing; plugin install.
