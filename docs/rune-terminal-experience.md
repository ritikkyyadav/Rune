...
> which are one bundle (`docs/rune-desktop-design.md`), and this surface stays
...
Rune is a calm coding workbench, not a feed of agent internals. The pigments
...
(`docs/design/rune-customizer.html` and `rune-customizer-v2.html` were the
...
untouched. Every retired accent id (`rune-violet-dark`, `orange`, `flow`, …)
...
The four-row, nine-tooth Rune avatar, actual version, active model/provider, current
...
```text
  ...
  ──────────────────────────────────────────────────────────────
  › Give Rune a coding task (or type / for commands)…
...
has a task. Narrow terminals collapse to a one-line `⚙︎ Rune` lockup while
...
`/sessions`, `/mode`, and `/diff`. It then exposes the wider Rune command set,
...
Rune and sandbox changes are announced in the transcript because they alter
...
- Text-bearing tokens meet WCAG AA contrast on explicit Rune surfaces.
...
- `packages/orchestrator/src/bin/ui/banner.ts` — four-row, nine-tooth Rune avatar and live metadata.
