...
2. **builds `apps/web` and generates the embed manifest** — a binary compiled without it cannot
   serve the product, so a missing `apps/web/dist/index.html` fails the job here (see below);
3. cross-compiles the CLI for five targets, version injected;
4. builds `gear-tools` natively on four runners plus a cross build for `linux-arm64`;
5. verifies every asset the target list names is present — an incomplete set fails here rather than
   becoming a half-broken release;
6. generates `SHA256SUMS` **in the same job** that assembled the binaries, so checksum drift is
   impossible by construction;
7. smoke-runs the Linux pair end to end (`--version`, a real `tools-smoke`, then `serve --check`);
8. **refuses to publish if the binary's version ≠ the tag**;
9. generates `Formula/gear.rb` into a `homebrew-tap` artifact from the checksums it just computed;
10. publishes the GitHub release with `--generate-notes`;
11. re-installs from the published release exactly the way the README instructs, and on all three
    operating systems runs `--version`, `doctor`, `tools-smoke`, the print smoke and
    `serve --check`.

---

## The web client ships inside the binary

There is one download per platform. `apps/web/dist` is **not** a release asset and never should be:
a downloaded `gear` has no source tree beside it, so the bundle is compiled into the executable.

`scripts/gen-web-embed.ts` reads the built `apps/web/dist` and writes
`packages/orchestrator/src/web-embed/generated.js` — one `import … with { type: "file" }` per asset,
which is the syntax `bun build --compile` looks for when deciding what to copy into the output, plus
a map from `dist`-relative path to the `/$bunfs/root/…` path the running binary can open. That file
is a build artifact: gitignored, prettier-ignored, regenerated by every path that compiles a binary.
What is committed is the generator and the `.d.ts` beside it that gives `tsc` the module's shape.

Precedence at runtime: an on-disk `apps/web/dist` **wins** when there is one, so a developer's
rebuild is live without a reinstall; the embedded copy answers when there is not, which is every
installed binary. Neither present is a hard refusal to start — never a server that answers `401` to
a browser.

Every compile path builds the client first and fails loudly without it:

| Path                       | Step                                                                                                                                           |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `scripts/install.sh`       | builds `apps/web`, generates the manifest, compiles, then proves the **staged** binary with `gear serve --check` before promoting it onto PATH |
| `scripts/build-release.sh` | builds `apps/web` and generates the manifest before the target loop                                                                            |
| `release.yml` (`cli` job)  | same, as its own step, with `::error::` on a missing `index.html`                                                                              |
| `ci.yml` (`packaged-e2e`)  | same, then runs `gear serve --check` on all three operating systems                                                                            |

## `gear serve --check`

The gate that would have caught both P10.9a defects. It runs **from the artifact**, which is the
point: every gate before it ran Gear from source with `bun`, where the bundle is a directory on disk
and `engine-host.ts` is a file that exists. It asserts, in order:

1. there is a client to serve, and `GET /` answers `200` with the page and the token already in it;
2. every asset that page references is served too (a map with one entry in it would otherwise pass);
3. a session opens over the WebSocket and a prompt reaches `turn_complete` against a mock provider
   that needs no credential — which can only happen if an engine host actually spawned;
4. shutdown leaves no host behind.

```bash
gear serve --check          # the installed binary
gear serve --check --port N # pin the port instead of taking an ephemeral one
```

It uses a throwaway `GEAR_HOME` and workspace, so it never touches real sessions.
