...
the answer into the binary with `bun build --define=RUNE_BUILD_VERSION=...`.

```
1. $RUNE_VERSION            an explicit override (CI passes the tag)
...
# 2. Bump the semver in packages/*/package.json.
...
3. builds `rune-tools` natively on four runners plus a cross build for `linux-arm64`;
...
6. smoke-runs the Linux pair end to end (`--version`, a real `tools-smoke`, then `serve --check`);
7. **refuses to publish if the binary's version ≠ the tag**;
8. generates `Formula/rune.rb` into a `homebrew-tap` artifact from the checksums it just computed;
9. publishes the GitHub release with `--generate-notes`;
10. re-installs from the published release exactly the way the README instructs, and on all three
    operating systems runs `--version`, `doctor`, `tools-smoke`, the print smoke and
    `serve --check`.
...
The gate that would have caught the P10.9a defect. It runs **from the artifact**, which is the
point: every gate before it ran Rune from source with `bun`, where
`engine-host.ts` is a file that exists. It asserts, in order:

1. the server comes up and mints a token;
2. a session opens over the WebSocket and a prompt reaches `turn_complete` against a mock provider
   that needs no credential — which can only happen if an engine host actually spawned;
3. shutdown leaves no host behind.

```bash
rune serve --check          # the installed binary
rune serve --check --port N # pin the port instead of taking an ephemeral one
```
...
binary untouched. `RUNE_INSTALL_DIR` (POSIX) and `-InstallDir` (Windows) relocate the install.
...
rune upgrade            # download, verify, install; keeps one .backup
```

Rune looks at the latest release at most once a day in the background and the only result is one line
at startup. Nothing is ever replaced without the command being typed. `[update] check = false` in
`~/.rune/config.toml` turns the background look off entirely.
...
`savoir/rune-releases`) that holds only the two install scripts and the release assets, add a step to
...
`get.rune.dev` vanity URL is a CNAME plus a redirect and is purely cosmetic.

### D1b — The Homebrew tap

`release.yml` generates `Formula/rune.rb` and uploads it as a `homebrew-tap` artifact. It does not
...
```bash
brew install --build-from-source savoir/tap/rune && rune --version && rune tools-smoke
```
...
| `RUNE_SIGNING_PRIVATE_KEY` | Ed25519 private key (base64) for detached Linux signatures              | `bun run scripts/keygen.ts`, or any Ed25519 keypair — see `packages/orchestrator/src/signing.ts` |
...
private half into `RUNE_SIGNING_PRIVATE_KEY`, put the public half in the README's verification block
...
`rune tools-smoke` and one real task, and write down what happened. Record each in
...
rune run shell commands without the containment macOS and Linux provide. `install.ps1` says this out
