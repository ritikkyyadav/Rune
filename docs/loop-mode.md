...
- A bare `/loop` uses `.gear/loop.md`, then `~/.gear/loop.md`, then Gear's bounded maintenance prompt.
...
- `.gear/loop.md` files are capped at 25,000 bytes. Symlinked prompt files are ignored so a project
