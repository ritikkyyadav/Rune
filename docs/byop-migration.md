...
2. **`~/.alan/secrets.json` users are unchanged.** Keys in the legacy secrets
...
```
secure store (keychain / OAuth token)  →  ~/.alan/secrets.json  →  config.toml  →  env var
```
