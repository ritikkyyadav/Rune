...
| D2  | The handoff "state of work" block was set down verbatim — up to 346 columns on an 80-column window. The fixed frame clips rather than reflows, so the one block a run that _died_ owes the reader was guaranteed to be cut off mid-sentence.                                                 | `ui/events.ts`   | 119 overflowing rows                               | 0                            | `ui-handoff-wrap.test.ts`       |
