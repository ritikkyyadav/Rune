#!/usr/bin/env bun
// ─── Berne telemetry collector (the receiving end) ───
// A single, dependency-free Bun server you run on your own machine or a small
// box. It receives the opt-in reports Berne clients POST, stores them in a
...
//   • Ingest is guarded by a shared bearer token (BERNE_COLLECTOR_TOKEN).
//   • No external dependencies, no outbound calls — inspect it in one read.
//
// Run:
//   BERNE_COLLECTOR_TOKEN=your-secret bun collector/berne-collector.ts
//
// Env:
//   BERNE_COLLECTOR_PORT   default 8787
//   BERNE_COLLECTOR_TOKEN  shared secret; if set, POST /ingest requires it
//   BERNE_COLLECTOR_DB     default ~/.berne-collector/reports.db
//
// Point a Berne build at it by setting, in the client's ~/.alan/config.toml:
//   [telemetry]
//   enabled  = true
...
const PORT = Number(process.env.BERNE_COLLECTOR_PORT ?? 8787);
const TOKEN = process.env.BERNE_COLLECTOR_TOKEN ?? "";
const DB_PATH =
  process.env.BERNE_COLLECTOR_DB ?? join(homedir(), ".berne-collector", "reports.db");
...
function dashboardHtml(): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Berne · telemetry</title>
...
console.log(`
  ...
  ingest    : POST http://localhost:${server.port}/ingest   ${TOKEN ? "(token required)" : "(open — set BERNE_COLLECTOR_TOKEN!)"}
  stats     : http://localhost:${server.port}/stats
  database  : ${DB_PATH}

  Point Berne clients at this via [telemetry] endpoint/token in ~/.alan/config.toml.
`);
