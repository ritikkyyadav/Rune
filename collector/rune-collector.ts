#!/usr/bin/env bun
// ─── Rune telemetry collector (the receiving end) ───
// A single, dependency-free Bun server you run on your own machine or a small
// box. It receives the opt-in reports Rune clients POST, stores them in a
...
//   • Ingest is guarded by a shared bearer token (RUNE_COLLECTOR_TOKEN).
//   • No external dependencies, no outbound calls — inspect it in one read.
//
// Run:
//   RUNE_COLLECTOR_TOKEN=your-secret bun collector/rune-collector.ts
//
// Env:
//   RUNE_COLLECTOR_PORT   default 8787
//   RUNE_COLLECTOR_TOKEN  shared secret; if set, POST /ingest requires it
//   RUNE_COLLECTOR_DB     default ~/.rune-collector/reports.db
//
// Point a Rune build at it by setting, in the client's ~/.rune/config.toml:
//   [telemetry]
//   enabled  = true
...
const PORT = Number(process.env.RUNE_COLLECTOR_PORT ?? 8787);
const TOKEN = process.env.RUNE_COLLECTOR_TOKEN ?? "";
const DB_PATH =
  process.env.RUNE_COLLECTOR_DB ??
  process.env.GEAR_COLLECTOR_DB ??
  join(homedir(), ".rune-collector", "reports.db");

mkdirSync(dirname(DB_PATH), { recursive: true });
// Continuity for upgraded deployments: a collector that previously ran under
// the Gear name kept its reports at ~/.gear-collector. When the new default
...
if (!process.env.RUNE_COLLECTOR_DB && !process.env.GEAR_COLLECTOR_DB && !existsSync(DB_PATH)) {
  const legacy = join(homedir(), ".gear-collector", "reports.db");
...
function dashboardHtml(): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Rune · telemetry</title>
...
console.log(`
  ...
  ingest    : POST http://localhost:${server.port}/ingest   ${TOKEN ? "(token required)" : "(open — set RUNE_COLLECTOR_TOKEN!)"}
  stats     : http://localhost:${server.port}/stats
  database  : ${DB_PATH}

  Point Rune clients at this via [telemetry] endpoint/token in ~/.rune/config.toml.
`);
