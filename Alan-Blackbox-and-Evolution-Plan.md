# ALAN — FEATURE PLAN: BLACK BOX (F1) & EVOLUTION LOOP (F2)

**Scope:** Alan CLI first. Desktop/engine-host wiring is a follow-on note, not in scope here.
...
3. **Every incident answers five questions:** *where* (component + code site), *what* (class + message + stack), *when* (session/turn/seq/timestamp/version), *why* (context: model, provider, tokens, tool, retries, config), *what Alan did* (the trail of events leading in — and what happened next).
4. **Struggles ≠ exceptions.** Thrash, abandonment, user corrections, and edit churn are first-class incidents even when no error was thrown. This is where "Alan breaks on varied codebases" actually lives.
...
| `tool.*` | exec_failure, invalid_input, timeout, sandbox_denial, path_violation, mcp_server_crash, mcp_protocol_error, background_shell_lost | tool-registry execute wrapper; MCP client error paths; `alan-tools` exit codes + stderr via `rust-bridge.ts` |
...
1. **Tool middleware (one chokepoint).** The tool-registry execute wrapper sees every tool call and result (`success=false` → incident; nonzero bash exit → incident; sandbox denial parsed from `alan-tools` stderr → incident). This single tap covers the entire `tool.*` family, including MCP and background shells.
...
**Recorder self-protection:** every public recorder function catches internally; on internal failure it writes one line to `~/.alan/blackbox.last-resort.log` and disables the failing sink for the run. `alan doctor` reports recorder health — the black box must never be the thing that silently broke.

## 1.5 Struggle detection (the half that finds "Alan struggles with varied codebases")
...
- **`alan doctor`** — health snapshot: recorder status, crash sentinel, last 7 days incident counts by class/severity, top 5 fingerprints, provider fallback rates, sandbox status.
- **`alan incidents [list|show <id>|top|export]`** — browse and inspect; `show` renders the trail; `top` ranks fingerprints; `export` writes a **redacted** bundle (JSONL + reports) for filing an issue — this is how a user reports reliably without knowing what matters.
...
- **Version-over-version:** `alan incidents top --by-version` diffs fingerprints across versions: *new in this version / fixed since last / regressed*. This is the sentence the user asked for — "so that with updated versions the system could be updated with required refinements" — made into a query.
...
**F1 exit criteria:** all four tap types live; chaos suite green on mac+linux CI; `alan doctor`/`incidents`/`/bug` shipped; a real dogfood week where every observed misbehavior (however small) is findable in `alan incidents` afterwards — that last one is the actual acceptance test, run deliberately.
...
Alan evolves the way you meant it: **an evolving system, not a cyborg.** Concretely:
...
5. **Corruption is survivable:** schema-versioned stores; on corruption, quarantine the file, log a `crash.evolution_store` incident, boot pristine. Alan must never fail to start because its brain is bruised.

## 2.6 Inspection — `alan brain` (auditability is the brand)

- `alan brain` — overview: counts by layer/status, token budget in use, evolution lift (last measured).
- `alan brain list [facts|lessons|skills|policies] [--repo|--global]`
- `alan brain why <id>` — full lineage: born from incidents A, B (linkable to `alan incidents show`), fired in sessions X, Y, win-rate curve, transitions. *"Why does Alan believe this?" always has an answer* — the same tamper-evident ethos as the audit chain and signed exports.
- `alan brain disable|delete <id>`, `alan brain export` (markdown), `evolution.enabled=false` master switch, per-layer switches.
...
Named loop sites + StruggleDetector + outcome resolver + `alan doctor` / `alan incidents` / `/bug`.
**Exit:** the dogfood-week acceptance test — every misbehavior observed while using Alan on real work is findable in `alan incidents`.
...
Per-repo facts store wiring `EpisodicMemory` + R2.4 discovery events; budgeted injection; freshness invalidation; `alan brain` (read-only first).
...
**Verify:** `alan brain why` on the promoted lesson tells a true, complete story.
...
- **Lane A (recorder/F1 core):** `packages/telemetry/**`, `packages/shared` (types, `onSalvage` seam), gateway tap, tool-middleware tap, `alan-tools` panic hook.
...
| Risk | Mitigation |
| --- | --- |
| Recorder becomes the crash | Self-protection wrapper, last-resort log, overhead bench in CI, `alan doctor` self-report |
