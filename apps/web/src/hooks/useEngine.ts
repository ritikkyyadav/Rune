...
/** Failed attempts before a loopback page reloads itself for a fresh token. */
const RELOAD_AFTER_ATTEMPTS = 2;
...
export function useEngine(options: UseEngineOptions) {
  ...
  const [transportKind, setTransportKind] = useState<"ws" | "none">(() =>
    configuredServer() ? "ws" : "none",
  );
   ...
   * dropped socket.
   *
   * `gear serve --web` mints a new token every start, so a page holding the old
   * one would retry forever against a door whose lock changed. After two failed
   * attempts — enough to distinguish a restart from a blip — a page that was
   * served its token by the engine simply asks for itself again, and the fresh
   * page arrives with the current token embedded. A page whose token came from
   * a pasted link cannot do that (a reload loses the fragment), so it keeps
   * retrying and the connection line says what is happening.
   */
