...
//!
//! `GEAR_SHELL` overrides the choice on any platform, because someone will have
...
fn from_override() -> Option<Shell> {
    let raw = std::env::var("GEAR_SHELL").ok()?;
...
mod tests {
    ...
    fn posix_is_bin_sh() {
        ...
        if std::env::var("GEAR_SHELL").is_err() {
            let sh = command_shell();
    ...
    fn windows_never_resolves_to_bin_sh() {
        ...
        // failed with os error 3 for every command Gear tried to run.
        let sh = command_shell();
