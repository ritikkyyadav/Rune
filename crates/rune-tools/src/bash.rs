...
pub async fn execute(input: BashInput, workspace_root: &Path) -> Result<BashOutput, ToolError> {
    ...
    // command at least RUNS. See rune_sandbox::shell.
    let sh = rune_sandbox::command_shell();
    ...
    // CLI) must kill this child's whole process group, not just rune-tools.
    rune_sandbox::active_child::set(child_pid, true);
    ...
    .await;
    rune_sandbox::active_child::clear();
...
pub async fn execute_sandboxed(
    ...
    use rune_sandbox::{SandboxConfig, create_sandbox};
        ...
        .await
    {
        ...
        Err(rune_sandbox::SandboxError::Timeout(ms)) => Ok(BashOutput {
            stdout: String::new(),
            stderr: format!(
...
mod tests {
    ...
    async fn inherits_parent_environment() {
        ...
        .await
        .unwrap();
        unsafe { std::env::remove_var("RUNE_BASH_ENV_PROBE") };
