...
// `rune serve --status` would swallow the next argument as its value. Rewrite
// it to a positional before parsing, so `rune serve --status` and
// `rune serve status` mean the same thing and neither eats a following flag.
...
const { values, positionals } = parseArgs({
  ...
  options: {
    ...
    // `rune detach --worktree`: isolate the run in a git worktree checkout.
    worktree: { type: "boolean", default: false },
    // `rune mcp` / `rune plugin`. Declared even though parsing is non-strict:
    ...
    // `rune serve`: the websocket transport (see docs/protocol.md).
    port: { type: "string" },
    host: { type: "string" },
    origin: { type: "string" },
    ...
    // `rune attach ws://host:port`: the bearer token, and an optional prompt to
    // run on the remote engine. `RUNE_SERVE_TOKEN` is preferred over --token,
    // which lands in shell history.
    token: { type: "string" },
    prompt: { type: "string" },
    session: { type: "string" },
    // `rune pr <n>`: review rather than work on it, and print the brief
    // instead of starting a session.
    review: { type: "boolean", default: false },
    brief: { type: "boolean", default: false },
    // `rune serve --check` / `rune upgrade --check`: a headless proof, nothing installed.
    check: { type: "boolean", default: false },
    // `rune audit <session> --record`: the Decision Record as Markdown, and
...
// `rune tui` and `rune console` name the default explicitly. The terminal is
// the product, so both are the bare command with the word dropped.
...
// are queryable per release. Mirrors the public brand version (Rune v0.1).
const RUNE_VERSION = PRODUCT_VERSION;
  ...
  process.stdout.write(
    terminalText(
      `\n  ${PRODUCT_LABEL} — AI coding agent\n\n` +
        `  Usage:\n` +
        `    rune                          Start the console — offers to resume recent work\n` +
        `    rune --new                    Skip the picker and start a fresh session\n` +
        `    rune resume [sessionId]       Resume a session (no id → interactive picker)\n` +
        `    rune list [--all]             List stored sessions (--all includes archived)\n` +
        `    rune export <sessionId>       Export a session transcript\n` +
        `    rune detach "<prompt>"        Start a background run that survives this terminal (--worktree isolates it)\n` +
        `    rune attach [session|latest]  Reattach to a detached run — replay, live-stream, Ctrl+C detaches again\n` +
        `    rune attach ws://host:port    Attach to a remote \`rune serve\` (--token or RUNE_SERVE_TOKEN, --prompt runs a turn)\n` +
        `    rune serve [--status]         The engine as a server for \`rune attach ws://\` and the SDK; --status describes a running one (--keep-hosts leaves session engines up)\n` +
        `    rune serve --check            Prove THIS binary can host a session: one turn over the socket, no host left behind\n` +
        `    rune pr <n> [--review]        Check a pull request out into its own worktree and start on it (--brief prints the brief)\n` +
        `    rune acp                      Agent Client Protocol server on stdio — for Zed and other ACP editors (docs/editors.md)\n` +
        `    rune login [provider]         Authenticate a provider — API key, or OAuth where supported (--method, --no-browser, --migrate)\n` +
...
if (command === "tools-smoke") {
  ...
  if (!lookup.found) {
    console.error(
      `  ${glyph("failure")} rune-tools not found -- set RUNE_TOOLS_BIN, re-run scripts/install.sh, or \`cargo build --release -p rune-tools\``,
    );
    process.exit(1);
  }
...
if (command === "attach") {
  // `rune attach ws://host:port` is the same command over the websocket
  ...
  if (/^wss?:\/\//i.test(target)) {
    ...
    process.exit(
      await runAttachRemote(target, {
        ...
        // `rune attach ws://… "do the thing"` and `--prompt "…"` mean the same.
...
if (command === "engine-host") {
  // The session-host entry. `rune serve` and `rune detach` spawn THIS binary as
  // `rune engine-host`, so nobody needs Bun or a source checkout on their
  // machine. Importing the module runs it: engine-host.ts owns stdio (or
  ...
  // input closes; falling through would start a second, interactive Rune on
...
if (command === "pr") {
  // `rune pr <n>`: the PR head in its own worktree, with the author's
...
function providerKeyEntryMap(
  secrets: ReturnType<typeof loadSecrets>,
): Record<string, import("@rune/shared").StoredKey[]> {
  const out: Record<string, import("@rune/shared").StoredKey[]> = {};
...
/**
 * Locate the Rust tool executor. Order: RUNE_TOOLS_BIN (legacy ALAN_TOOLS_BIN is
 * adopted at startup) → the source checkout's target/{release,debug}/rune-tools
 * → ~/.rune/bin/rune-tools (installed next to the launcher) → `rune-tools` on
...
/** Resolve `rune resume <id>` / `--resume <id>` to a session, reconciling its model+provider. */
...
/** `rune resume` with no id: a numbered picker on a TTY, most-recent on a pipe. */
...
// Asked at most once (persisted in ~/.rune/telemetry.json), only on an
...
async function askTelemetryConsent(): Promise<boolean> {
  process.stdout.write(
    `\n  ${bold(text("Help improve Rune?"))}\n` +
      ...
      `  ${faint("· Inspect the exact payloads any time:")} ${info("rune telemetry preview")}\n` +
      `  ${faint("· Change your mind any time:")} ${info("rune telemetry on|off")}\n\n`,
  );
  const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
  try {
...
async function main() {
  // Rename migration: RUNE_* env → RUNE_*, ~/.alan → ~/.rune (once, with a note).
  adoptLegacyEnv();
  const homeMigrationNote = migrateLegacyHome();
  if (homeMigrationNote) console.error(`  ${dim(`rune: ${homeMigrationNote}`)}`);
  ...
  if (updateNag) console.error(`  ${dim(`rune: ${updateNag}`)}`);
  const dataDir = ensureDataDir();
  const toolsLookup = await findToolsBinary();
  const toolsBinary = toolsLookup.path;
  if (!toolsLookup.found) {
    console.error(
      `  ${vermillion("⚠")} ${dim("rune-tools binary not found — set RUNE_TOOLS_BIN, re-run scripts/install.sh, or `cargo build --release -p rune-tools`; file and shell tools will fail until then (`rune doctor` shows what was searched).")}`,
    );
  }
  ...
  // pre-BYOP for anyone who never runs `rune login`.
  ...
  const engine = new Engine({
    ...
    // `rune models`, because a session that boots on `bedrock` needs the same
    ...
    // it off). Unit tests construct the Engine directly and stay hermetic.
    // The trail spool is pid-scoped so two concurrent Rune instances don't
    ...
    blackbox:
      config.diagnostics?.enabled !== false
        ? {
            ...
            spoolPath: joinPath(getRuneHome(), `blackbox.spool.${process.pid}.json`),
          }
        : undefined,
  ...
  if (command === "list" || values.list) {
    ...
    if (sessions.length === 0) {
      ...
      if (archivedCount > 0) {
        console.log(
          `\n  ${dim(`+ ${archivedCount} archived/deleted — `)}${dim("rune list --all")}`,
        );
      }
      console.log(
        `\n  ${dim("Resume: ")}${cyanotype("rune resume")}${dim(" (picker) or ")}${cyanotype("rune resume <id>")}\n`,
      );
    }
    engine.close();
    return;
  ...
  if (command === "export") {
    ...
    if (!sessionId) {
      process.stderr.write(
        `  ${vermillion("✕")} Usage: rune export <sessionId> [--format md|json] [--sign] [--out <path>]\n`,
      );
      engine.close();
      process.exit(1);
    }
  ...
  // markers whose owner pid is DEAD, so concurrent Rune tabs never file
  ...
  const sentinelDir = joinPath(getRuneHome(), "sentinels");
  const ownSentinel = sentinelPathFor(sentinelDir, process.pid);
  const ownSpool = joinPath(getRuneHome(), `blackbox.spool.${process.pid}.json`);
  if (recorder) {
    ...
    const dirtyExits = sweepDirtyExits(sentinelDir, {
      legacyPath: joinPath(getRuneHome(), "blackbox.sentinel.json"),
    });
    ...
    // Only sweep old ones so a concurrently running rune isn't clobbered.
    ...
    process.on("exit", () => {
      ...
      // spools don't accumulate in ~/.rune.
      try {
  ...
  // reads it verbatim. Progress goes to stderr so `rune -P "..." > out.txt`
  ...
  // the user's terminal recoloured after Rune quits.
  applyTerminalTheme();
  ...
  // When launched with --resume / `rune resume`, replay the prior conversation so
  ...
  const SLASH_CMDS: [string, string][] = [
    ...
    ["/undo", "Revert the last Rune auto-commit ([git] autoCommit)"],
  ...
  async function handleInput(input: string, scheduledLoop?: LoopTask) {
    ...
    if (!scheduledLoop) {
      ...
      if (input === "/keys" || input.startsWith("/keys ")) {
        ...
        const printTable = () => {
          process.stdout.write(
            `  ${bold(text("API keys"))} ${faint("· saved to ~/.rune/secrets.json, applied live")}\n` +
              `  ${faint("· OAuth & OS-keychain storage:")} ${info("rune login <provider>")} ${faint("· status:")} ${info("rune providers")}\n\n`,
          );
      ...
      if (input === "/notebook") {
        ...
        if (entries.length === 0) {
          process.stdout.write(
            `  ${dim("Notebook is empty for this workspace — Rune fills it as it verifies how your repos work.")}\n\n`,
          );
        } else {
          ...
          process.stdout.write(
            `\n  ${dim("manage: rune notebook [show <id>|rm <id>|export]")}\n\n`,
          );
        }
        showPrompt();
        return;
      ...
      if (input === "/bug" || input.startsWith("/bug ")) {
        ...
        if (!rec) {
          ...
          process.stdout.write(
            id
              ? `  ${green("✦")} ${text("Logged with the current flight trail.")} ${faint(`· rune incidents show ${id.slice(-8)}`)}\n\n`
              : `  ${dim("Could not record — see rune doctor.")}\n\n`,
          );
        }
        showPrompt();
        return;
            ...
            if (accounts.length > 1) {
              ...
              if (row.source === "oauth" || row.source === "keychain") {
                if (account.kind === "key" || account.kind === "env") {
                  process.stdout.write(
                    `  ${dim(`note: the signed-in ${row.source} credential wins on the wire —`)} ${info(`rune logout ${chosen.id}`)} ${dim("to use API keys")}\n`,
                  );
                }
      ...
      // Custom slash commands from .rune/commands/*.md — render, then run as a prompt.
