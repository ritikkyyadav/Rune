...
import type { AlanConfig, SecretsFile, AuthMethod, CredentialStore } from "@alan/shared";
 ...
 * merely echo an env var, then ~/.alan/secrets.json (which wins). Kept here so
...
export function readAuthOverrides(config: AlanConfig): Record<string, AuthMethod> {
...
export function insecureNoticeLine(store: CredentialStore): string | null {
  ...
  return "⚠ credentials stored unencrypted at ~/.alan/credentials.json (no OS keychain available)";
}
