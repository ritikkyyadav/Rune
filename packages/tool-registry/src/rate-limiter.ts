...
export class ToolRateLimiter {
  ...
  checkLimit(toolName: string): { allowed: boolean; retryAfterMs?: number } {
    ...
    const limit = toolName === 'bash' ? this.config.bashMaxPerMinute
      : ['write_file', 'edit_file'].includes(toolName) ? this.config.writeMaxPerMinute
  ...
  reset(): void { this.calls.clear(); this.globalCalls = []; }

  getStats(): Record<string, number> {
    const now = Date.now();
    const windowStart = now - 60_000;
    const stats: Record<string, number> = { _global: this.globalCalls.filter(t => t > windowStart).length };
