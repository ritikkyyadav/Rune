...
// single file, a second concurrent Rune instance would "consume" the first
...
import type { TrailEntry } from "@rune/shared";
 ...
 * a concurrently running Rune is not a crash. Each consumed marker yields
...
export function sweepDirtyExits(
  ...
  for (const path of candidates) {
    try {
      ...
      if (alive(meta.pid)) continue; // another Rune is running — leave its marker
