...
| <!-- P12.1 --> _run economics on free tiers: lane P12.1 fills this in_ <br> **The MCP client against real local servers** — filesystem, memory and everything over stdio, streamable HTTP and SSE on 2026-09-08 ([record](docs/evidence/mcp-live-20260908.md)); the GitHub connector and vendor OAuth stay unverified | **User skills, hooks and plugin install** — `/skills`, `/<name>`, `rune skill add`, the hooks fixture and `rune plugin add` of the two examples run through the real CLI and engine against the scripted mock provider; the TUI panel has not been watched on a real terminal | **Research mode's answer quality**, and **workflows** — both run, neither has been scored _(experimental)_                                                                                                |
