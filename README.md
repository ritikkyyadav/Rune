...
| Install from source on macOS, and the published v0.4.0 one-liner into a fresh `HOME` on macOS (assets verified against `SHA256SUMS`); `rune doctor`, `rune tools-smoke`, `rune serve --check`; a detached nine-file task with tests on a free route ([record](docs/evidence/live-run-long-20260908.md)) | Linux — the v0.4.0 fresh-machine install job was green in the release workflow on 2026-09-08; Windows — the same job **failed** at "version, doctor, tools, and a real prompt" and the log has not been read                                                                                                                                                                                                                                                                                                        | **External benchmarks** — the SWE-bench Verified 50 and Terminal-Bench 20 subsets are pinned and the commands are printed; **zero runs recorded**                                                                                                                                 |
...
**v0.4.0 is the first release the one-line installers can fetch.** Releases up to v0.3.1 carry
`gear-*` assets and the installers look for `rune-*`. The v0.4.0 one-liner was run into a fresh
`HOME` on macOS on 2026-09-08 and installed a working binary with verified checksums; the Windows
fresh-machine smoke in that release's workflow failed and has not been read yet, so treat the
Windows package as unverified.
...
**Prebuilt** — macOS and Linux verified for v0.4.0; Windows unverified:
