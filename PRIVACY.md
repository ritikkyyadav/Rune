...
- **Off by default.** Nothing leaves your machine until (a) the build is
  ...
  first-run prompt (or run `berne telemetry on`).
- **You can see the exact bytes.** `berne telemetry preview` prints the complete
  set of payloads that could ever be sent — no hidden fields.
- **Change your mind any time.** `berne telemetry off` stops it; `berne telemetry
...
— no MAC address, hostname, or hardware serial — and `berne telemetry reset`
replaces it.
...
it is unavoidable for any network request. Berne's design response is: **the IP
is never placed in a telemetry payload**, and the reference collector
(`collector/berne-collector.ts`) **never stores it** — at most it is passed to an
...
| Command | Effect |
| --- | --- |
| `berne telemetry status` | Show config, consent, install id, and what's queued |
| `berne telemetry preview` | Print the exact payloads that could be sent |
| `berne telemetry on` / `off` | Opt in / out |
| `berne telemetry reset` | Forget the decision + install id |

Config lives under `[telemetry]` in `~/.alan/config.toml` (or a project
`.alan/config.toml`); env overrides `ALAN_TELEMETRY`,
`ALAN_TELEMETRY_ENDPOINT`, `ALAN_TELEMETRY_TOKEN`. Setting `enabled = false` or
