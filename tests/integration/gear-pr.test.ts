/**
 * `gear pr <n>`, run against a pull request that exists.
 *
...
describe("gear pr <n> (a real fetch, a real worktree, a fake pull request)", () => {
  let dir: string;
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "gear-pr-"));
    ...
    // is the whole trick: `gear pr` needs nothing else from a remote.
  ...
  function fakeApi(): string {
    api = Bun.serve({
      ...
      fetch: (req) => {
        ...
        if (pathname !== `/repos/savoir/gear/pulls/${PR_NUMBER}`) {
  ...
  test("fetches the head into its own worktree and hands the session the author's brief", async () => {
    ...
    const worktree = join(local, ".gear", "worktrees", `pr-${PR_NUMBER}`);
    expect(out).toContain(worktree);
    expect(out).toContain(`gear/pr-${PR_NUMBER}`);
    ...
    // `rev-parse --abbrev-ref`: the latter answers `heads/gear/pr-N` for a
    // branch name with a slash in it.
    expect(git(worktree, ["symbolic-ref", "--short", "HEAD"])).toBe(`gear/pr-${PR_NUMBER}`);
    ...
    // Tracked files only: the command does add `.gear/worktrees/pr-N`, which is
    // untracked in a repository that has not ignored `.gear/` (backlog).
  ...
  test("re-running after a push moves the worktree instead of refusing", async () => {
    ...
    // "move it to the new head" logic could run. The second `gear pr 12` — the
