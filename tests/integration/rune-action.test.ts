 ...
 * `rune audit`, compose the comment — and prints the comment instead of
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "rune-action-"));
  repo = join(dir, "repo");
...
describe("the action's pieces", () => {
  ...
  test("reads the pull request number out of the event payload", () => {
    ...
    expect(inputs.repo).toBe("savoir/rune");
  });
  ...
  test("a failed run is reported as a failed run, not as an empty review", () => {
    ...
    expect(comment).toContain("<!-- rune-review -->");
  });

  test("strips ANSI out of the audit page", () => {
    const painted = "  [36mRune audit[0m [2m01a0[0m  fix the thing";
    expect(auditSummary(painted)).toBe("Rune audit 01a0  fix the thing");
  });
  ...
  test("a mock run says it is one, before it says anything else", () => {
    ...
    expect(comment).toContain("### Rune review — dry run (mock provider)");
...
describe("the action end to end, against a fake model", () => {
  test.skipIf(!HAS_RUST_BIN)(
    ...
    async () => {
      ...
      expect(code, `stderr:\n${err}`).toBe(0);
      expect(out).toContain("<!-- rune-review -->");
      expect(out).toContain("### Rune review");
      ...
      expect(out).toContain("Rune audit");
  ...
  test.skipIf(!HAS_RUST_BIN)(
    ...
    async () => {
      // The mode `.github/workflows/rune-review.yml` uses when
      // `RUNE_REVIEW_API_KEY` is absent, which on this repository is always.
      ...
      expect(out).toContain("### Rune review — dry run (mock provider)");
