/**
 * `rune attach ws://host:port` — the parts that decide what credential goes
...
import { lanAddresses } from "../../../packages/orchestrator/src/bin/serve-cli";

const localFile = { token: "from-the-file", url: "ws://127.0.0.1:4762" };

describe("which token rune attach uses", () => {
  test("--token wins", () => {
    const got = resolveToken("ws://10.0.0.5:7788", "flag-token", { RUNE_SERVE_TOKEN: "env" }, null);
    expect(got).toEqual({ token: "flag-token", from: "--token" });
  });

  test("RUNE_SERVE_TOKEN when there is no flag", () => {
    const got = resolveToken("ws://10.0.0.5:7788", undefined, { RUNE_SERVE_TOKEN: "env" }, null);
    expect(got).toEqual({ token: "env", from: "RUNE_SERVE_TOKEN" });
  });
  ...
  test("no token at all is a refusal, not a guess", () => {
    ...
    expect(resolveToken("ws://10.0.0.5:7788", "", { RUNE_SERVE_TOKEN: "" }, null)).toBeNull();
  });
  ...
  test("knows a loopback URL from a LAN one", () => {
    ...
    for (const u of ["ws://10.0.0.5:1", "wss://rune.example:443", "not a url"]) {
