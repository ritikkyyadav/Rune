 ...
 *     accountId, refresh.
 *   • toResponsesInput / toResponsesBody — Rune request → Responses API shape.
 *   • parseResponsesStream — Responses SSE → Rune StreamEvents.
...
describe("toResponsesInput / toResponsesBody", () => {
  ...
  it("puts the system prompt in instructions and sets reasoning for a codex model", () => {
    ...
    expect(body.instructions).toBe("You are Rune.");
...
describe("parseResponsesStream", () => {
  it("translates a text + tool-call + completion stream into Rune events", async () => {
    const events = [
