...
Neither program is loaded into the Gear process. Each is spawned under
Seatbelt (macOS) or bubblewrap (Linux) with exactly the capability its manifest
entry declares, and Gear refuses to start either one on a machine with no
...
The tools then appear as `plugin_gear-example-tools_write_text`,
`plugin_gear-example-tools_read_text` and `plugin_gear-example-tools_http_get`.
...
bundle off with `plugin:gear-example-tools:*`.
